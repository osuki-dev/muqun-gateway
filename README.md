...
The file type is decided by sniffing the content; the client's filename is only
echoed back and never reaches the filesystem. png, jpeg, gif, webp, heic, pdf,
and plain text (txt, md, log, json, csv) are accepted. Executables and scripts
are refused by magic number. The body is capped at 25 MiB, and stored uploads
are deleted 48 hours after they were written.
...
- Uploads are typed by magic number, never by filename, and executables, Mach-O
  and ELF binaries, Windows `MZ` images, and `#!` scripts are refused outright.
